...
function loadSigninForm() {
  document.getElementById('signin-box').innerHTML = `
    <button class="signin-close" onclick="closeAllModals()">×</button>
    <h2>Sign In</h2>
    <form id="signin-form">
      ...
      <p style="margin-top: 5px; font-size: 14px;">
        <a href="#" onclick="loadForgotPassword()" style="color: black;">Forgot password?</a>
      </p>
...
document.addEventListener('submit', function (e) {
  if (e.target.id === 'signin-form') {
    ...
    if (!email.endsWith('@wmsu.edu.ph')) {
      document.getElementById('signin-email-error').textContent = 'Email must end with @wmsu.edu.ph';
      valid = false;
    }
...
document.addEventListener('click', function (e) {
  if (e.target.id === 'send-code-btn') {
    ...
    if (!email.endsWith('@wmsu.edu.ph')) {
      document.getElementById('forgot-email-error').textContent = 'Email must end with @wmsu.edu.ph';
      return;
    }
...
// Confirm Code Button
document.addEventListener('click', function (e) {
  if (e.target.id === 'confirm-code-btn') {
  ...
  if (e.target.id === 'reset-password-btn') {
    ...
    if (valid) {
      document.getElementById('forgot-form').innerHTML = `
        ...
        <button onclick="loadSigninForm()">Back to Sign In</button>
      `;
    }
  }
});
...
function loadSignupForm() {
  document.getElementById('signup-box').innerHTML = `
    <button class="signup-close" onclick="closeAllModals()">×</button>
    <h2>Sign Up</h2>
    <form id="signup-form">
      ...
      <p style="margin-top: 10px; font-size: 14px;">Student ID number:</p>
...
document.addEventListener('submit', function (e) {
  if (e.target.id === 'signup-form') {
    ...
    if (!email.endsWith('@wmsu.edu.ph')) {
      document.getElementById('signup-email-error').textContent = 'Email must end with @wmsu.edu.ph';
      valid = false;
    }
    ...
    if (!/^\d{4}$/.test(prefix) || !/^\d{5}$/.test(suffix)) {
      document.getElementById('signup-student-id-error').textContent = 'Student ID format invalid';
      valid = false;
    }
